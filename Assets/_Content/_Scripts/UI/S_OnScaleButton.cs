...
public class OnScaleButton : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IPointerEnterHandler, IPointerExitHandler
{
	...
	{
		m_graphicsParent.transform.DOScale(m_scaleDownMultiplier, m_scaleDuration).SetEase(Ease.OutBack);
		if (m_imgBlack != null) m_imgBlack.DOFade(0.75f, 0);
	}

	public void OnPointerUp(PointerEventData data)
	{
		m_graphicsParent.transform.DOScale(Vector3.one, m_scaleDuration).SetEase(Ease.OutBack);
		if (m_imgBlack != null) m_imgBlack.DOFade(0, 0);
		OnClick.Invoke();
	}

	public void OnPointerEnter(PointerEventData eventData)
	{
		m_graphicsParent.transform.DOScale(m_scaleUpMultiplier, m_scaleDuration).SetEase(Ease.OutBack);
	}
