...
public static class Matha
{
	...
	public static float SimplexNoise(Vector3 vector)
	{
        ...
        DVector3 v = new DVector3(vector);
		DVector3 di = DVector3.Floor(v + DVector3.Dot(v, new DVector3(CD[1], CD[1], CD[1]) * DVector3.one));
        DVector3 dx0 = v - di + DVector3.Dot(di, new DVector3(CD[0], CD[0], CD[0])) * DVector3.one;
        // ------- TEST DOUBLE ---------
		...
		Vector3 i2 = Vector3.Min(g, new Vector3(l.z, l.x, l.y));
	...
	public static Vector4 InvSqrt(Vector4 vector)
	{
		return Vector4.one * 1.79284291400159f - (vector * 0.85373472095314f);
    }

	private static void PrecisionPrint(DVector3 vector)
	{
        Debug.Log("(" + vector.x.ToString() + ", " + vector.y.ToString() + ", " + vector.z.ToString() + ")");
    }

    private static void PrecisionPrint(Vector4 vector)
    {
